...
lm_entry_income_filtered <- lm(norm_entries ~ income, data = station_race16)
summary(lm_entry_income_filtered)

# Plotting
ggplot(filtered_data) +
  geom_point(aes(income, norm_entries)) +
  geom_line(aes(income, predict(lm_entry_income_filtered, newdata = filtered_data))) +
  ggtitle('Normalized Gated Entries vs. Income') +
  xlab('median income')+
  ylab('filtered normalized gated entries')
```
...
filtered_data <- subset(station_race16, norm_entries < 125)
# Filter data for income more than 125000 (too less data points)
filtered_data <- subset(filtered_data, race == "Black")

lm_entry_income_filtered <- lm(norm_entries ~ percent, data = station_race16)
summary(lm_entry_income_filtered)
...
ggplot(filtered_data) +
  ...
  geom_line(aes(percent, predict(lm_entry_income_filtered, newdata = filtered_data))) +
  ggtitle('Normalized Gated Entries vs. Race') +
  xlab('How White')+
  ylab('filtered normalized gated entries')
```
```{r}
lm_entry_income <- lm(norm_entries ~ race, data = station_race16)
summary(lm_entry_income)
```
```{r}
...
```{r}
ggplot()+geom_point(aes(station_race16$race,station_race16$norm_entries))+
  geom_line(aes(station_race16$race, predict(lm_entry_income, newdata = station_race16)))+
  ggtitle('normalized gated entries vs. race')+
  xlab('normalized gated entries')+
  ylab('median race')
```
...
for (i in 1:length(station_populations_16_22$station_name)){
  ...
  station_populations_16_22$diff_norm_entry[i] <- (norm_entry_22 - norm_entry_16) / norm_entry_16 * 100
}
...
diff_norm_station_entry <- ggplot() +
  ...
  scale_fill_gradient2(low = "red", mid = "yellow", high = "green", midpoint = 200, na.value = NA, name = "Percentage of Rider Increase in 2022")
