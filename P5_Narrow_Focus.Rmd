...
df_sum <- aggregate(gated_entries ~ service_date, data = df, FUN = sum)

# Create a line plot of sum of entries per date for all stations
ggplot(df_sum, aes(x = service_date, y = gated_entries)) +
  geom_line() +
  labs(x = "Date", y = "Total Gated Entries",
       title = "Total Gated Entries per Day for All Stations") +
  theme_minimal() +
...
df_january <- df[df$month == "2014-01", ]

# Aggregate the data by station and calculate the sum of entries for January
df_january_sum <- df_january %>%
  group_by(station_name) %>%
  summarise(total_entries = sum(gated_entries))

# Create a bar graph of total entries in January for each station
ggplot(df_january_sum, aes(x = station_name, y = total_entries)) +
  geom_bar(stat = "identity", fill = "skyblue") +
  labs(x = "Station Name", y = "Total Entries in January",
       title = "Total Entries in January for Each Station") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
```
\
