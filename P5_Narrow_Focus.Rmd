...
- data source: MBTA Gated Station Entries - Historical
- data description - what's being measured?: The count of gated station entries for each day for multiple years broken up in 30 minute increments
- data limitations (eg: are the data recent? do they contain all variables you might want?): The most recent data is two months old. This only contains ridership data for days and not data on any of the other things we are looking into.
- data dimensions - how much data do you have?: There are eleven csv files dating back to 2014. Each pof them has at least a million entries of six variables (excel won't show the entire thing)
- how might the data be joined with other data you have?: date, time, and type of transportation could be combined with other data.

- data source: MBTA fuel and energy
- data description - what's being measured?: The volume, miles traveled and efficiency of different energy sources used by the MBTA for different services over time.
- data limitations (eg: are the data recent? do they contain all variables you might want?): This was most recently updated in 2021. It does not contain information on ridership, but it can be extrapolated by the miles traveled.
- data dimensions - how much data do you have?: 288 observations of 8 variables
- how might the data be joined with other data you have?: year and type of transportation could be combined with other data.
...
energy_data <- read.csv("Fuel_and_Energy.csv") %>% 
  filter(Volume > 0, Miles_Traveled != "NA") %>% 
  select(-Efficiency, -ObjectId) %>% 
  mutate(Efficiency = Volume/Miles_Traveled) %>% 
  group_by(Mode, Units)



ggplot(energy_data, aes(x=Year, y=Efficiency)) +
  geom_point(aes(color=Mode)) +
  facet_grid("Fuel_Source", scales = "free_y") +
  labs(y="Efficiency (Volume/Miles Traveled)")
  theme()
  
```
it seems that the use of a gasoline has changed in efficiency over time, becoming more efficient an compressed natural gas is less efficient.
\
...
we could gather more data about economic distribution across gated stations and or routes. The visualizations help to allow us to see what aspects of these questions should be pursued, such as comparing gated entries to total ridership data.
...
we collectively worked on the first portion of the assignment then split to create our visualizations.
